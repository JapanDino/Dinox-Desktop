...
import { DndContext, DragOverlay, PointerSensor, useSensor, useSensors, useDroppable, useDraggable, rectIntersection, type DragEndEvent, type DragStartEvent, } from "@dnd-kit/core";
import { SortableContext, horizontalListSortingStrategy, useSortable, arrayMove, } from "@dnd-kit/sortable";
import { CSS } from "@dnd-kit/utilities";
import { CloudSun, BatteryFull, Cpu, MemoryStick, HardDrive, ArrowUpDown, X, ArrowLeftRight, ChevronUp, ChevronDown, } from "lucide-react";
...
const WIDGET_DEFS: WidgetDef[] = [
    { id: "weather", get label() {
            return tr("Weather");
        }, icon: CloudSun, color: "#60a5fa" },
    { id: "battery", get label() {
            return tr("Battery");
        }, icon: BatteryFull, color: "#4ade80" },
    { id: "cpu", label: "CPU", icon: Cpu, color: "#f97316" },
    { id: "ram", label: "RAM", icon: MemoryStick, color: "#a78bfa" },
    { id: "disk", get label() {
            return tr("Disk");
        }, icon: HardDrive, color: "#38bdf8" },
    { id: "net", get label() {
            return tr("Net");
        }, icon: ArrowUpDown, color: "#2dd4bf" },
];
...
function PoolChip({ id }: {
    ...
    const Icon = def.icon;
    return (<div ref={setNodeRef} className={`widget-pill widget-pill--available ${isDragging ? "dragging" : ""}`} {...listeners} {...attributes}>
      <Icon size={12} strokeWidth={2} style={{ color: def.color }}/>
      <span>{def.label}</span>
    </div>);
}
/* ── Sortable placed chip ── */
function SortablePlacedChip({ id, idx, total, onSwap, onRemove, onMove, }: {
    id: string;
    idx: number;
    total: number;
    ...
    const Icon = def.icon;
    return (<div ref={setNodeRef} style={style} className={`widget-pill widget-pill--placed ${isDragging ? "dragging" : ""}`}>
      <div {...attributes} {...listeners} className="widget-pill-drag-handle">
        <Icon size={12} strokeWidth={2} style={{ color: def.color }}/>
        <span>{def.label}</span>
      </div>
      <div className="widget-pill-btns">
        {idx > 0 && (<button className="widget-pill-btn" onClick={() => onMove(id, -1)}>
            <ChevronUp size={9}/>
          </button>)}
        {idx < total - 1 && (<button className="widget-pill-btn" onClick={() => onMove(id, 1)}>
            <ChevronDown size={9}/>
          </button>)}
        <button className="widget-pill-btn" title={tr("Swap side")} onClick={() => onSwap(id)}>
          <ArrowLeftRight size={9}/>
        </button>
        <button className="widget-pill-btn widget-pill-btn--x" title={tr("Remove")} onClick={() => onRemove(id)}>
          <X size={10}/>
        </button>
      </div>
    </div>);
...
/* ── Droppable zone ── */
function DropZone({ id, side, items, onSwap, onRemove, onMove, }: {
    id: string;
    side: "left" | "right";
    items: string[];
    onSwap: (id: string) => void;
    ...
    return (<div ref={setNodeRef} className={`widget-config-zone ${isOver ? "widget-config-zone--over" : ""}`}>
      <span className="widget-config-side">{side === "left" ? tr("Left") : tr("Right")}</span>
      <div className="widget-config-chips">
        <SortableContext items={items} strategy={horizontalListSortingStrategy}>
          {items.length > 0
            ? items.map((itemId, i) => (<SortablePlacedChip key={itemId} id={itemId} idx={i} total={items.length} onSwap={onSwap} onRemove={onRemove} onMove={onMove}/>))
            : <span className="widget-config-empty">{tr("Drop here")}</span>}
        </SortableContext>
      </div>
    </div>);
}
...
export function StatusWidgetConfig({ value, onChange }: StatusWidgetConfigProps) {
    ...
    const getZoneOf = (itemId: string): "left" | "right" | "pool" | null => {
        if (config.left.includes(itemId))
            return "left";
        if (config.right.includes(itemId))
            return "right";
    ...
    const handleDragEnd = useCallback((event: DragEndEvent) => {
        ...
        if (isPoolItem && targetZone) {
            if (config[targetZone].length >= MAX_PER_ZONE)
                return;
        ...
        if (isPoolItem && !targetZone && active.rect.current.initial) {
            ...
            for (const side of ["left", "right"] as const) {
                if (config[side].length >= MAX_PER_ZONE)
                    continue;
    ...
    return (<DndContext sensors={sensors} collisionDetection={rectIntersection} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="widget-config">
        <DropZone id="zone-left" side="left" items={config.left} onSwap={swapSide} onRemove={remove} onMove={moveInSide}/>
        <DropZone id="zone-right" side="right" items={config.right} onSwap={swapSide} onRemove={remove} onMove={moveInSide}/>

        {pool.length > 0 && (<div className="widget-config-pool">
            <div className="widget-config-pool-chips">
              {pool.map((id) => (<PoolChip key={id} id={id}/>))}
            </div>
          </div>)}
      </div>

      <DragOverlay>
        {activeDef ? (<div className="widget-pill widget-pill--dragging">
