...
function TitleMarquee({ title }: {
    ...
    return (<div ref={containerRef} className="cmp-marquee-wrap">
      <span ref={textRef} className={`cmp-title ${isOverflowing ? 'cmp-marquee' : ''}`} style={{
...
export function CompactMediaPlayer({ mediaInfo, albumArtUrl, albumArtKey, isPlaying, volume, volumeExpanded, onVolumeExpandedChange, onTogglePlayPause, onVolumeChange, prevFront, prevBack, nextFront, nextBack, onAnimatePrev, onAnimateNext, onLayoutChange, }: CompactMediaPlayerProps) {
    ...
    useEffect(() => {
        const tick = () => {
            if (isSeekingRef.current)
                return; // don't update during seek
    ...
    return (<div className="cmp">
      ...
      <div className="cmp-row cmp-row-info">
        <div className="cmp-art" onClick={(e) => { e.stopPropagation(); onLayoutChange('classic'); }} style={{ cursor: 'pointer' }}>
          <AnimatePresence mode="wait" initial={false}>
            {albumArtUrl ? (<motion.img key={`art-${albumArtKey}`} src={albumArtUrl} alt={tr("Art")} draggable={false} initial={{ rotateY: 90, opacity: 0 }} animate={{ rotateY: 0, opacity: 1 }} exit={{ rotateY: -90, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }}/>) : (<motion.div key="placeholder" className="cmp-art-placeholder" initial={{ rotateY: 90, opacity: 0 }} animate={{ rotateY: 0, opacity: 1 }} exit={{ rotateY: -90, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }}>
      ...
      {/* Row 2: Progress bar */}
      {durationMs > 0 && (<div className="cmp-row cmp-row-progress">
          ...
          <div className="cmp-track-bar">
            ...
            {seekEnabled && (<input ref={seekRef} type="range" className="cmp-seek" min={0} max={durationMs} step={100} value={displayPosition} onMouseDown={handleSeekStart} onTouchStart={handleSeekStart} onChange={handleSeekChange} onMouseUp={handleSeekEnd} onTouchEnd={handleSeekEnd} onClick={(e) => e.stopPropagation()}/>)}
          </div>
      ...
      <div className="cmp-row cmp-row-controls">
        <motion.button className="cmp-btn cmp-btn-side" onClick={(e) => { e.stopPropagation(); onVolumeExpandedChange(!volumeExpanded); }} whileTap={{ scale: 0.9 }} title={tr("Volume")}>
        ...
        <div className="cmp-main-controls">
          ...
          <motion.button className="cmp-btn cmp-play" onClick={(e) => { e.stopPropagation(); onTogglePlayPause(); }} whileTap={{ scale: 0.95 }}>
            <AnimatePresence mode="wait" initial={false}>
              <motion.div key={isPlaying ? "pause" : "play"} initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.7 }} transition={{ duration: 0.12 }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <motion.button className="cmp-btn cmp-btn-side" onClick={(e) => {
            e.stopPropagation();
            invoke('open_sound_settings').catch(() => { });
        }} whileTap={{ scale: 0.9 }} title={tr("Audio Output")}>
      ...
      {/* Volume expansion */}
      <AnimatePresence>
        {volumeExpanded && (<motion.div className="cmp-expand" initial={{ height: 0, opacity: 0 }} animate={{ height: 36, opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }}>
            <div className="cmp-volume-row">
              ...
              <div className="cmp-volume-track">
                ...
                <input type="range" className="cmp-volume-slider" min={0} max={1} step={0.01} value={volume} onChange={(e) => onVolumeChange(parseFloat(e.target.value))} onClick={(e) => e.stopPropagation()}/>
              </div>
