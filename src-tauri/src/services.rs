...
unsafe extern "system" fn focus_event_proc(
    ...
    if hwnd.0.is_null() { return; }
    crate::keyboard_layout::remember(hwnd);
        ...
        unsafe {
            ...
            loop {
                let _: Result<(), String> = (|| {
                    ...
                    let mut eco_enabled = crate::commands::personal_power_state(app_handle.clone());
                    let mut last_eco_check = Instant::now();
                    let mut last_device_check = std::time::Instant::now();
                    loop {
                        if last_eco_check.elapsed() >= Duration::from_secs(2) {
                            eco_enabled = crate::commands::personal_power_state(app_handle.clone());
                            last_eco_check = Instant::now();
                        }
                        let visualizer_enabled = crate::utils::get_setting_str(&app_handle, "bloom-media-visualizer-enabled")
                            .or_else(|| crate::utils::get_setting_str(&app_handle, "bloom-visualizer-enabled"))
                            .is_none_or(|value| value != "false");
                        let paused = eco_enabled || !visualizer_enabled || !ANY_MEDIA_PLAYING.load(Ordering::Relaxed);
                        if paused {
                            if capture_running { let _ = audio_client.Stop(); let _ = audio_client.Reset(); capture_running = false; buffer_pos = 0; }
                            std::thread::sleep(Duration::from_millis(500));
                        } else {
                            if !capture_running { audio_client.Start().map_err(|_| "Audio restart failed")?; capture_running = true; }
...
pub fn setup_system_worker(app_handle: AppHandle) -> Sender<SystemCommand> {
    ...
    std::thread::spawn(move || {
        ...
        unsafe {
            ...
            loop {
                ...
                CURRENT_AUDIO_READY.store(audio_endpoint_volume.is_some(), Ordering::Relaxed);
                if let Some(ref aev) = audio_endpoint_volume {
                    if let (Ok(vol), Ok(muted)) = (aev.GetMasterVolumeLevelScalar(), aev.GetMute()) {
                        let is_muted: bool = muted.into();
                        CURRENT_MUTED.store(is_muted, Ordering::Relaxed);
    ...
    std::thread::spawn(move || {
        ...
        let initial = wmi_con.query::<WmiMonitorBrightness>().ok().and_then(|res| res.first().map(|b| b.current_brightness as u32));
        BRIGHTNESS_AVAILABLE.store(initial.is_some(), Ordering::Relaxed);
        let mut last_brightness = initial.unwrap_or(50);
        ...
        loop {
            ...
            if let Ok(results) = wmi_con.query::<WmiMonitorBrightness>() {
                BRIGHTNESS_AVAILABLE.store(!results.is_empty(), Ordering::Relaxed);
        ...
        loop {
            unsafe {
                ...
                while !hwnd.is_invalid() && check_count < 15 {
                    ...
                    let is_bloom = process_id == my_process_id || class_str.contains("Dinox");
        ...
        if let Some(app_handle) = MOUSE_HOOK_APP_HANDLE.get() {
            ...
            if fg_fs {
                ...
                if dock_win.is_visible().unwrap_or(false) {
                    ...
                    let should_ignore = crate::input_policy::ignore_cursor(crate::input_policy::Surface::Dock,is_click_interactive,MENU_IS_OPEN.load(Ordering::Relaxed));
            ...
            if !fg_fs {
                if let Some(main_win) = app_handle.get_webview_window("main") {
                    if main_win.is_visible().unwrap_or(false) {
                        ...
                        let final_ignore = crate::input_policy::ignore_cursor(crate::input_policy::Surface::Main,is_click_interactive,MENU_IS_OPEN.load(Ordering::Relaxed));
...
static TOP_APPBAR_RECT: std::sync::Mutex<Option<(isize,i32,i32,i32,i32)>>=std::sync::Mutex::new(None);
pub fn register_appbar(window: tauri::WebviewWindow) {
    if CURRENT_FOREGROUND_FULLSCREEN.load(Ordering::Relaxed) {return;}
    let reserve = get_setting_str(window.app_handle(), "bloom-notch-mode").as_deref().unwrap_or("fixed") == "fixed"
        && get_setting_str(window.app_handle(), "bloom-notch-reserve-space").as_deref() == Some("true");
    if !reserve {
        if let Ok(hwnd) = window.hwnd() {
            if MAIN_APPBAR_REGISTERED.swap(false, Ordering::Relaxed) { unregister_appbar_native(hwnd); }
            *TOP_APPBAR_RECT.lock().unwrap_or_else(|e|e.into_inner())=None;
            if let Ok(Some(monitor)) = window.primary_monitor() {
                let height = ((760.0 * crate::utils::get_bloom_scale(window.app_handle()) * monitor.scale_factor()) as u32).min(monitor.size().height);
                let _ = window.set_position(*monitor.position());
                let _ = window.set_size(tauri::PhysicalSize::new(monitor.size().width, height));
            }
            re_assert_topmost(hwnd);
        }
        return;
    ...
    if let Ok(Some(monitor)) = window.app_handle().primary_monitor() {
        ...
        let ph = (((760.0 * bloom_scale) * scale) as i32).min(m_size.height as i32);
        ...
        unsafe {
            ...
            abd.uCallbackMessage = appbar_callback_message();
            install_appbar_callback(hwnd);
            if !MAIN_APPBAR_REGISTERED.load(Ordering::Relaxed) {
                if SHAppBarMessage(ABM_NEW, &mut abd) == 0 { return; }
            ...
            abd.rc.bottom=abd.rc.top+pr;
            let target=(hwnd.0 as isize,abd.rc.left,abd.rc.top,abd.rc.right,abd.rc.bottom);
            if TOP_APPBAR_RECT.lock().unwrap_or_else(|e|e.into_inner()).as_ref()!=Some(&target) {
                SHAppBarMessage(ABM_SETPOS, &mut abd);
                *TOP_APPBAR_RECT.lock().unwrap_or_else(|e|e.into_inner())=Some(target);
            }
...
pub fn register_dock_appbar(window: tauri::WebviewWindow) {
    crate::shell_controller::request_refresh(window.app_handle());
}
...
pub unsafe extern "system" fn enum_windows_proc(hwnd: HWND, lparam: LPARAM) -> BOOL {
    ...
    if IsWindowVisible(hwnd).as_bool() {
        ...
        if len > 0 {
            ...
            if title == "Program Manager" || title == "Dinox" || title == "Dinox Dock" {
                return true.into();
            }
...
fn reposition_all_windows(app_handle: &AppHandle) {
    ...
    crate::shell_controller::request_refresh(app_handle);
...
pub fn setup_display_change_monitor(app_handle: AppHandle) {
    ...
    std::thread::spawn(|| {
        unsafe {
            ...
            let class_name = windows::core::PCSTR(c"DinoxDisplayMonitor".as_ptr() as *const u8);
            ...
            use windows::Win32::UI::Input::KeyboardAndMouse::{RegisterHotKey, MOD_CONTROL, MOD_ALT, MOD_NOREPEAT};
...
unsafe extern "system" fn display_monitor_proc(
    ...
    if msg == taskbar_created {
        if let Some(app)=DISPLAY_MONITOR_HANDLE.get() {
            crate::shell_controller::explorer_restarted(app);
            if let Some(main)=app.get_webview_window("main") { register_appbar(main); }
        }
        return LRESULT(0);
    }
    if msg == WM_HOTKEY && wparam.0 == 0xB100 {
        if let Some(app)=DISPLAY_MONITOR_HANDLE.get() {
    ...
    if shell_message != 0 && msg == shell_message {
        if let Some(app) = DISPLAY_MONITOR_HANDLE.get() {
            // HSHELL_FLASH = HSHELL_REDRAW | HSHELL_HIGHBIT. No notification text is read.
            let event = match wparam.0 {
                0x8006 => Some("dock-attention"),
                4 | 0x8004 | 2 => Some("dock-attention-clear"),
                _ => None,
            };
            if let Some(event) = event { let _ = app.emit_to("dock", event, lparam.0); }
        }
        return LRESULT(0);
    }
...
pub fn appbar_callback_message() -> u32 {
    unsafe { windows::Win32::UI::WindowsAndMessaging::RegisterWindowMessageW(windows::core::w!("BloomPersonal.AppbarChanged")) }
}

pub fn install_appbar_callback(hwnd: HWND) {
    // SetWindowSubclass must run on the window's owning UI thread.
    if let Some(app)=DISPLAY_MONITOR_HANDLE.get() {
        let raw=hwnd.0 as isize;
        let _=app.run_on_main_thread(move || unsafe {
            let _=windows::Win32::UI::Shell::SetWindowSubclass(HWND(raw as *mut _),Some(appbar_proc),0xB100,0);
        });
    }
}

unsafe extern "system" fn appbar_proc(hwnd: HWND,msg:u32,wp:windows::Win32::Foundation::WPARAM,lp:windows::Win32::Foundation::LPARAM,_id:usize,_data:usize)->windows::Win32::Foundation::LRESULT {
    use windows::Win32::UI::Shell::{DefSubclassProc,ABN_POSCHANGED,ABN_FULLSCREENAPP};
    if msg==appbar_callback_message() && wp.0==ABN_FULLSCREENAPP as usize {
        if lp.0!=0 {
        ...
        } else { re_assert_topmost(hwnd); }
    }
    if msg==appbar_callback_message() && wp.0==ABN_POSCHANGED as usize {
        if let Some(app)=DISPLAY_MONITOR_HANDLE.get() {
            crate::shell_controller::request_refresh(app);
        }
    }
