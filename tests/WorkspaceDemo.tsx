...
export function WorkspaceDemo({send}:{send:()=>void}){
 ...
 return <div className="workspace-demo"><header className="demo-header"><a href="?workspace&notifications-allowed" className="demo-wordmark">dinox<span>DESKTOP / 4</span></a><span className="demo-browser-label">{tr('Браузерная демоверсия · Windows не изменяется')}</span><a href="?settings&notifications-allowed" className="demo-settings-link">{tr('Все настройки')}<ArrowUpRight size={15}/></a></header>
  <main className="demo-main"><aside className="demo-intro"><span className="demo-eyebrow">{tr('Твоё пространство')}</span><h1>{tr('Меньше шума.')}<br/><span>{tr('Больше своего.')}</span></h1><p>{tr('Приложения под рукой, спокойные уведомления и единый стиль с календарём.')}</p><nav aria-label={tr('Разделы демоверсии')}>{[[PanelBottom,'dock','Мой док'],[Bell,'notices','Уведомления'],[CalendarDays,'calendar','Календарь'],[CalendarDays,'connections','Подключения'],[Monitor,'performance','Производительность']] .map(([Icon,id,title]:any)=><button key={id} aria-pressed={page===id} onClick={()=>setPage(id)}><Icon size={18}/>{tr(title)}</button>)}</nav><div className="demo-test"><Monitor size={18}/><p>{tr('Все устройства и сообщения здесь демонстрационные. Нажатия не управляют компьютером.')}</p><button disabled={!notifications.options.enabled} onClick={send}><Send size={16}/>{tr('Тестовое сообщение Telegram')}</button><small>{tr('Проверяет карточку, счётчик и внимание значка одним событием.')}</small></div></aside>
  <section className={`demo-panel ${page==='calendar'?'demo-calendar':''}`} aria-label={tr('Настройка Dinox')}>{page==='dock'?<><DockContentSettings/><details className="demo-appearance"><summary>{tr('Размер, отступы и внешний вид')}</summary><DockDesignSettings/></details></>:page==='notices'?<NotificationSettings/>:page==='connections'?<CalendarSettings/>:page==='performance'?<PerformanceOptions/>:<CalendarPanel eco={eco} model={calendar} date={date} onDate={setDate} pinned={pin} onPinned={()=>setPin(!pin)}/>}</section></main>
